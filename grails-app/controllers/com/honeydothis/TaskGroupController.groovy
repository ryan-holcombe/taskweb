...
class TaskGroupController
{
  ...
  def save = {
    params.user = User.findById(springSecurityService.principal.id);
    def taskGroupInstance = new TaskGroup(params)
    if (taskGroupInstance.save(flush: true))
    {
      flash.message = "${message(code: 'default.created.message', args: [taskGroupInstance.name])}"
  ...
  def delete = {
    ...
    if (taskGroupInstance)
    {
      try
      {
        ...
        flash.message = "${message(code: 'default.deleted.message', args: [message(code: 'taskGroup.label', default: 'TaskGroup'), taskGroupInstance.name])}"
        redirect(action: "list")
      }
  ...
  def deleteDialog = {
    ...
    if (taskGroupInstance)
    {
      render(view: "deleteDialog", model: [taskGroupInstance: taskGroupInstance])
    }
    else
    {
      flash.message = "${message(code: 'default.not.found.message', args: [message(code: 'taskGroup.label', default: 'TaskGroup'), params.id])}"
